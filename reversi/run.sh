...
map="$WK_DIR/data/map3.txt" #地图由此改 位于data文件夹下
...
# default time gap is 500000
./bin/judge --data_file="$map" --player_red="$computer" --player_blue="$player" --log_dir="$log_dir" --visible="$ok" --timegap=50000 || exit -1
